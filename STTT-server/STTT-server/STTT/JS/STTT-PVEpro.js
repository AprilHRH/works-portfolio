document.addEventListener('DOMContentLoaded', () => {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            for (let j = 0; j < 3; j++) {
                ...
                if (smallBoardWinners[index] === aiSymbol) {
                    ...
                    playerCount++;
                } else if (smallBoardWinners[index] === null) {
                    // 评估小棋盘的潜在价值
                    aiPotential += evaluateSmallBoardPotential(index, aiSymbol);
                    playerPotential += evaluateSmallBoardPotential(index, playerSymbol);
                }
            }
            ...
            if (aiCount > 0 && playerCount === 0) {
                ...
                // 如果AI有获胜机会，额外加分
                if (aiCount === 2 && aiPotential > 0) {
                    score += 100;
                }
            } else if (playerCount > 0 && aiCount === 0) {
                score -= playerCount * playerCount * 50;
                // 如果玩家有获胜机会，额外减分
                if (playerCount === 2 && playerPotential > 0) {
                    score -= 100;
                }
            }
        }
    ...
    function evaluateSmallBoardPotential(boardIndex, player) {
        const board = smallBoards[boardIndex];
        let potential = 0;
        ...
        for (let i = 0; i < 3; i++) {
            ...
            for (let j = 0; j < 3; j++) {
                if (board[row + j] === player) {
                    playerCount++;
                } else if (board[row + j] === (player === aiSymbol ? playerSymbol : aiSymbol)) {
                    opponentCount++;
                } else if (!board[row + j]) {
                    emptyCount++;
                }
            ...
            if (playerCount > 0 && opponentCount === 0) {
                potential += playerCount * playerCount * 5;
            }
        }
        ...
        for (let i = 0; i < 3; i++) {
            ...
            for (let j = 0; j < 3; j++) {
                if (board[i + j * 3] === player) {
                    playerCount++;
                } else if (board[i + j * 3] === (player === aiSymbol ? playerSymbol : aiSymbol)) {
                    opponentCount++;
                } else if (!board[i + j * 3]) {
                    emptyCount++;
                }
        ...
        for (let i = 0; i < 3; i++) {
            if (board[i * 4] === player) {
                playerCount++;
            } else if (board[i * 4] === (player === aiSymbol ? playerSymbol : aiSymbol)) {
                opponentCount++;
            } else if (!board[i * 4]) {
                emptyCount++;
            }
        ...
        for (let i = 0; i < 3; i++) {
            if (board[i * 2 + 2] === player) {
                playerCount++;
            } else if (board[i * 2 + 2] === (player === aiSymbol ? playerSymbol : aiSymbol)) {
                opponentCount++;
            } else if (!board[i * 2 + 2]) {
                emptyCount++;
            }
    ...
    function evaluateStrategicPosition(boardIndex, cellIndex) {
        let score = 0;
    ...
    function findBestMove(availableMoves) {
        ...
        for (const move of availableMoves) {
            ...
            // 位置权重
            score += positionWeights[cellIndex] * 2;
            
            // 潜在获胜评估（增强）
            score += evaluatePotentialWin(boardIndex, cellIndex, aiSymbol) * 3;
            score -= evaluatePotentialWin(boardIndex, cellIndex, playerSymbol) * 2.5;
            
            // 棋盘控制评估
            score += evaluateBoardControl(boardIndex, cellIndex) * 1.5;
            
            // 下一步棋盘优势评估
            score += evaluateNextBoardAdvantage(cellIndex) * 2;
            
            // 大棋盘局势评估
            score += evaluateBigBoardSituation(boardIndex, cellIndex) * 2;
            
            // 战略位置评估
            score += evaluateStrategicPosition(boardIndex, cellIndex);
    ...
    function evaluateBigBoardSituation(boardIndex, cellIndex) {
        let score = 0;
        
        // 如果这个移动能影响大棋盘的获胜路径，加分
        // 检查行
        const rowIndex = Math.floor(boardIndex / 3);
        for (let i = 0; i < 3; i++) {
            const index = rowIndex * 3 + i;
            if (index !== boardIndex && smallBoardWinners[index] === aiSymbol) {
                score += 10;
            } else if (index !== boardIndex && smallBoardWinners[index] === playerSymbol) {
                score -= 5;
            }
        }
        
        // 检查列
        const colIndex = boardIndex % 3;
        for (let i = 0; i < 3; i++) {
        ...
        // 检查对角线
        if (boardIndex === 0 || boardIndex === 4 || boardIndex === 8) {
            const diagIndices = [0, 4, 8];
            for (const index of diagIndices) {
                if (index !== boardIndex && smallBoardWinners[index] === aiSymbol) {
                    score += 10;
                } else if (index !== boardIndex && smallBoardWinners[index] === playerSymbol) {
                    score -= 5;
        ...
        if (boardIndex === 2 || boardIndex === 4 || boardIndex === 6) {
            const diagIndices = [2, 4, 6];
            for (const index of diagIndices) {
                if (index !== boardIndex && smallBoardWinners[index] === aiSymbol) {
                    score += 10;
                } else if (index !== boardIndex && smallBoardWinners[index] === playerSymbol) {
