...
addEventListener("keydown", (e) => {
  ...
  if (!keys[e.code]) {
    if (["Space", "ArrowUp", "KeyW"].includes(e.code)) jumpBuffered = 0.16;
  }
  keys[e.code] = true;
...
class CollapseFloor {
  constructor(rect, trigger, opts = {}) {
    ...
    this.shakeTime = Math.max(opts.shakeTime ?? 0.18, 0.28);
...
class PopSpikes {
  constructor(x, y, w, trigger, opts = {}) {
    ...
    this.delay = Math.max(opts.delay ?? 0, 0.14);
...
class FallBlock {
  constructor(rect, trigger, opts = {}) {
    this.home = { ...rect };
    this.trigger = trigger;
    this.shakeT = Math.max(opts.shakeTime ?? 0.12, 0.22);
...
class Crusher {
  constructor(x, w, opts = {}) {
    ...
    this.slamSpeed = opts.slamSpeed ?? 1250;
    this.upSpeed = opts.upSpeed ?? 220;
    this.holdT = Math.max(opts.hold ?? 0.32, 0.42);
    this.reset();
  }
  reset() {
    this.y = this.topY;
...
class CrumblePlatform {
  constructor(rect, opts = {}) {
    this.home = { ...rect };
    this.delay = Math.max(opts.delay ?? 0.35, 0.5);
    this.reset();
  }
  reset() { this.rect = { ...this.home }; this.state = "idle"; this.t = 0; this.vy = 0; }
  update(dt, g) {
...
const LEVELS = [
  ...
  // ---------------------------------------------------- 30 (NEW: invert + saw + crusher)
  {
    name: "MIRROR SELF",
    build: () => ({
      spawn: { x: 60, y: 440 },
      door: new Door([{ x: 884, y: 416 }]),
      solids: [floorSeg(0, 960), R(280, 320, 150, 16), roof(30), wallL(), wallR()],
      traps: [
        new InvertZone(R(180, 0, 460, 480)),
        new Saw([{ x: 600, y: 140 }, { x: 600, y: 350 }], { r: 21, speed: 185 }),
        new Crusher(720, 90, { topY: 30, period: 1.9, phase: 0.4 }),
        new PopSpikes(876, 480, 56, R(812, 330, 14, 150), { delay: 0.05 }),
        new Note(90, 420, "flip mode engaged"),
      ],
    }),
  },
  // ---------------------------------------------------- 31 (NEW: pendulums over conveyor)
  {
    name: "SWING SHIFT",
    build: () => ({
      spawn: { x: 60, y: 440 },
      ...
      solids: [floorSeg(0, 960), wallL(), wallR()],
      traps: [
        new Conveyor(R(130, 480, 560, 60), { dir: 1, force: 115 }),
        new Pendulum(270, 30, { len: 390, amp: 0.92, speed: 1.5, r: 17, phase: 0.0 }),
        new Pendulum(480, 30, { len: 390, amp: 0.92, speed: 1.5, r: 17, phase: 1.05 }),
        new Pendulum(690, 30, { len: 390, amp: 0.92, speed: 1.5, r: 17, phase: 2.1 }),
        new PopSpikes(806, 480, 64, R(750, 330, 16, 150), { delay: 0.05 }),
        new Note(90, 420, "mind the wrecking crew"),
      ],
    }),
  },
  // ---------------------------------------------------- 32 (NEW: button + gate + fake door)
  {
    name: "PUSH THE BUTTON",
    build: () => ({
      spawn: { x: 55, y: 440 },
      door: new Door([{ x: 884, y: 416 }]),
      solids: [floorSeg(0, 960), R(240, 330, 120, 14), wallL(), wallR()],
      traps: [
        new Button(295, 316, "a", { momentary: false }),
        new Gate(R(620, 300, 28, 180), "a"),
        new Saw([{ x: 480, y: 150 }, { x: 480, y: 360 }], { r: 20, speed: 170 }),
        new FakeDoor(790, 416, { label: "definitely real" }),
        new Note(140, 420, "the switch is up there"),
      ],
    }),
  },
  // ---------------------------------------------------- 33 (NEW: turret crossfire + springs)
  {
    name: "CROSSFIRE 2",
    build: () => ({
      spawn: { x: 55, y: 440 },
      door: new Door([{ x: 884, y: 416 }]),
      solids: [floorSeg(0, 300), floorSeg(460, 660), floorSeg(820, 960), wallL(), wallR()],
      traps: [
        new StaticSpikes(300, 540, 160, { dir: "up", size: 44 }),
        new StaticSpikes(660, 540, 160, { dir: "up", size: 44 }),
        new Spring(250, 480, { power: -1100 }),
        new Spring(610, 480, { power: -1100 }),
        new Turret(150, 340, { dir: 1, period: 1.4, speed: 310 }),
        new Turret(940, 340, { dir: -1, period: 1.4, speed: 310, phase: 0.7 }),
        new Note(150, 430, "dodge both directions"),
        new PopSpikes(880, 480, 52, R(826, 330, 14, 150), { delay: 0.05 }),
      ],
    }),
  },
  // ---------------------------------------------------- 34 (NEW: teleporter chain + laser)
  {
    name: "WARP LOGIC",
    build: () => ({
      spawn: { x: 55, y: 440 },
      door: new Door([{ x: 884, y: 226 }]),
      solids: [floorSeg(0, 260), floorSeg(430, 700), R(760, 260, 200, 16), wallL(), wallR()],
      traps: [
        new StaticSpikes(260, 540, 170, { dir: "up", size: 44 }),
        new BlinkPlatform(R(290, 420, 96, 16), { period: 1.7, onFrac: 0.62, phase: 0.1 }),
        new Teleporter(500, 432, 850, 222, { w: 30, h: 48, twoWay: false }),
        new Laser({ x: 620, y: 30, len: 418, vertical: true, period: 2.1, warn: 0.5, fire: 0.4, phase: 0.3 }),
        new PopSpikes(806, 260, 60, R(730, 180, 16, 80), { delay: 0.05 }),
        new Note(140, 430, "blink, warp, done"),
      ],
    }),
  },
  // ---------------------------------------------------- 35 (finale)
...
const Game = {
  ...
  updatePlay(dt) {
    ...
    const SPEED = 282;
    const accel = p.grounded ? 2800 : 2000;
    ...
    if (!heldJump() && p.jumping && p.vy < -240) { p.vy = -240; p.jumping = false; }

    p.vy = Math.min(p.vy + 2000 * dt, 980);
    ...
    for (const t of this.level.traps) {
      for (const k of t.kills()) {
        if (aabb(R(p.x + 6, p.y + 6, p.w - 12, p.h - 12), k)) { this.die(p.x + p.w / 2, p.y + p.h / 2); return; }
      }
    }
...
bindHold("tc-jump",
  () => { touch.jump = true; jumpBuffered = 0.16; },
  () => (touch.jump = false)
);
